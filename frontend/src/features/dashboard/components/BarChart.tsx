    ...
    <div className={cn("flex flex-col h-full", className)}>
      ...
      <div>
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col justify-between pointer-events-none" style={{ bottom: '32px', top: '0' }}>
            {ticks.reverse().map((tick) => (
              <div key={`tick-${tick}`} className="flex items-center gap-2 w-full">
                <span className="text-xs text-slate-500 tabular-nums w-6 text-right shrink-0">{tick}</span>
                <div className="flex-1 border-b border-dashed border-slate-800/60" />
              </div>
            ))}
          </div>
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      isHovered ? "opacity-100 text-slate-100" : "opacity-0"
                    )}
                  >
                    {segment.value}
                  </div>
          ...
          {/* X-axis labels */}
          <div className="flex justify-around pl-9 pt-2 border-t border-slate-800/60 mt-1">
            {segments.map((segment, i) => (
              <div
                key={segment.label}
                className={cn(
                  "flex flex-col items-center flex-1 transition-colors cursor-pointer",
                  hoveredIndex === i ? "text-slate-100" : "text-slate-500"
                )}
                onMouseEnter={() => setHoveredIndex(i)}
                onMouseLeave={() => setHoveredIndex(null)}
              >
